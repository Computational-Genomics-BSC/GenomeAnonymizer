...
def anonymize_genome(vcf_variants, normal_bam, tumor_bam, ref_genome, classifier, anonymizer: Anonymizer):
    """
    ...
    for variant_record in vcf_variants:
        ...
        normal_reads_pileup = normal_bam.pileup(reference=window[0], start=window[1], end=window[2],
                                                reference_genome=ref_genome)
        tumor_reads_pileup = tumor_bam.pileup(reference=window[0], start=window[1], end=window[2],
                                              reference_genome=ref_genome)
        anonymizer.anonymize(variant_record, normal_reads_pileup, tumor_reads_pileup, classifier)
...
def get_window(variant_record, window_size=2000):
    ...
    return variant_record.contig, variant_record.pos - half_window, variant_record.pos + half_window
