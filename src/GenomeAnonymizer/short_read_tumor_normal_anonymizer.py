        ...
        for dataset_idx, current_reads_stream in enumerate((tumor_reads_stream, normal_reads_stream)):
            for read_aln in current_reads_stream:
                ...
                if read_id in to_pair_anonymized_reads:
                    ...
                    updated_anonymized_read_pair1 = updated_anonymized_read_pair[PAIR_1_IDX]
                    updated_anonymized_read_pair2 = updated_anonymized_read_pair[PAIR_2_IDX]
                    print(f'To pair {read_id}: updated_anonymized_read_pair1 is None={updated_anonymized_read_pair1 is None} '
                          f' updated_anonymized_read_pair2 is None={updated_anonymized_read_pair2 is None}')
                    if updated_anonymized_read_pair1 is not None:
                              print(f' updated_anonymized_read_pair1 has only supplementary alignments={updated_anonymized_read_pair1.has_only_supplementary}')
                    if updated_anonymized_read_pair2 is not None:
                              print(f' updated_anonymized_read_pair2 has only supplementary alignments={updated_anonymized_read_pair2.has_only_supplementary}')
                    # TODO:Anonymize leftover variants in the read_aln pair from supplementary alignments
                    if anonymized_read_pair_is_writeable(updated_anonymized_read_pair1, updated_anonymized_read_pair2):
                        print(f'To pair {read_id}: writeable')
                        write_pair(indexed_pair_writer_streams, updated_anonymized_read_pair1,
                                   updated_anonymized_read_pair2)
                        to_pair_anonymized_reads.pop(read_id)
        close_paired_streams(indexed_pair_writer_streams)
